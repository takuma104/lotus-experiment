...
### 結果 6/6: `gsm-lotus-gpt2-full0-12` (全層 [0, 12) を mid-loop 実装で + add_norm 注入) — 2026-09-11 完了

学習 2026-09-10 08:41 → 09-11 21:35 (約 37 時間、stage 6 は約 1 時間/epoch)。注入モードの対照実験。

validation 精度の推移はベースラインとほぼ同じ (stage 1〜5 で 33〜35%、stage 6 で 37〜46%、best 45.6% @ epoch 27)。
**epoch 11 で一過性の loss spike** (grad norm が通常の約 4 から最大 134 へ、train loss 0.47 → 0.68、その epoch の
validation は 3.4% で latent 読み出しが「....」に退化) が起きたが、epoch 12 で 40.8% まで自然回復した。
勾配クリップ 1.0 でも lr 1e-4 定数スケジュールでは稀に spike する模様 (ベースラインでは未観測)。

| チェックポイント | GSM8K test | GSM-Hard | MultiArith | SVAMP | OOD 平均 |
| --- | --- | --- | --- | --- | --- |
| full0-12 checkpoint_final (best val, epoch 27) | 43.4% (573/1319) | 9.9 | 92.2 | 42.8 | 48.3 |
| full0-12 checkpoint_30 (最終 epoch) | 43.4% (573/1319) | 9.8 | 88.3 | 42.0 | 46.7 |
| 参考: full-legacy checkpoint_final (best val, epoch 27) | 43.7% (576/1319) | 9.9 | 91.1 | 42.1 | 47.7 |

→ **add_norm 注入そのものは全層ループでは無害** (ベースラインと同等)。したがって部分層ループ 4 変種の不振は
注入モードではなく「ループ対象を一部の層に限定したこと」に起因する。

## GPT-2 アブレーション まとめ (2026-09-04 〜 09-11)

### 精度 (GSM8K test / OOD 平均、%)

`checkpoint_final` は validation best による選択 (論文の規則)。部分層ループの 4 変種は stage 0 (latent なし) の
41.4% を一度も上回れなかったため `checkpoint_final` が stage 0 の重みになり評価不能 → 最終 epoch (checkpoint_30) で比較する。

| run | 層範囲 | val stage 0 | val best (stage ≥ 1) | val 最終 | checkpoint_final test / OOD | **checkpoint_30 test** / OOD | 学習ピーク GB | 学習時間 h |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| full-legacy | 0-12 (legacy) | 41.4 | 46.0 (ep 27) | 45.8 | 43.7 / 47.7 | **43.6** / 46.9 | 20.1 * | 39 |
| full0-12 | 0-12 | 41.4 | 45.6 (ep 27) | 43.4 | 43.4 / 48.3 | **43.4** / 46.7 | 19.3 | 37 |
| mid3-9 | 3-9 | 41.4 | 30.6 (ep 2) | 25.6 | (stage 0 重み) | **28.8** / 29.4 | 14.8 | 26 |
| mid4-8 | 4-8 | 41.4 | 32.0 (ep 2) | 26.2 | (stage 0 重み) | **26.9** / 25.6 | 13.3 | 23 |
| early0-4 | 0-4 | 41.4 | 30.0 (ep 2) | 24.2 | (stage 0 重み) | **25.2** / 23.6 | 13.3 | 23 |
| late8-12 | 8-12 | 41.4 | 31.0 (ep 2) | 24.0 | (stage 0 重み) | **25.9** / 23.6 | 13.3 | 23 |

論文 (Table 1, GPT-2 LOTUS, 3 seeds): GSM8K 44.1 ± 0.7、OOD 平均 47.9。No-CoT: 19.1。
\* `eval_batch_size` 修正前の epoch 1〜3 (validation-loss pass が batch 128) は 24.2 GB。修正後の定常値が 20.1 GB。

### 推論レイテンシ (GSM8K test、GPU アイドル状態で再計測、checkpoint_30、batch 1、fp32、RTX 5090)

| run | 層範囲 | thought ms/例 | 推論全体 ms/例 | test 精度 |
| --- | --- | --- | --- | --- |
| full-legacy | 0-12 (legacy) | 28.1 | 39.7 | 43.6 |
| full0-12 | 0-12 | 26.9 | 39.1 | 43.4 |
| mid3-9 | 3-9 | 17.2 | 29.1 | 28.8 |
| mid4-8 | 4-8 | 13.4 | 24.8 | 26.9 |
| early0-4 | 0-4 | 13.3 | 25.2 | 25.2 |
| late8-12 | 8-12 | 13.2 | 25.0 | 25.9 |

thought フェーズは「1 回の全層 pass + R 回の部分 pass」になるぶん部分層ループの方が速い (25% 幅で約 2.1 倍、50% 幅で約 1.6 倍) が、
精度を 15 ポイント以上失うため速度/精度のトレードオフとしては成立していない。

### 結論

1. **再現**: 1 GPU + 勾配累積 (16 × 8) の全層ループは論文の GPT-2 LOTUS を再現した (43.7% / 47.7 vs 44.1 ± 0.7 / 47.9)。
2. **中間層ループは GPT-2 の CoT 初期化 fine-tuning では機能しない**: middle 50% / middle 25% / early 25% / late 25% のどれも
   stage 2 (2 ステップ目を latent 化した時点) で 20% 前後に崩れ、stage 6 で 22〜28% のプラトーのまま終わる (test 25〜29%)。
   入口 (ℓ_s = 0 の early0-4) や出口 (ℓ_e = 12 の late8-12) を既存実装と揃えても変わらない。
3. **注入モードは原因ではない**: 全層 + add_norm (full0-12) はベースラインと同等 (43.4% / 48.3)。
4. 解釈: LOTUS の全層ループは「ln_f 後の最終隠れ状態 (tied embedding により次トークンのソフト埋め込みとみなせる) を
   入力に戻す」構造で、CoT 学習済みモデルが事前学習で持つ「入力 → 次ステップ表現」の写像をそのまま再帰ステップとして使える。
   部分層ループでは出口層の残差と入口層の入力空間に事前学習由来の対応が無く、30 epoch の fine-tuning では対応を獲得できなかった。
   T2MLR の中間層再帰の知見は from-scratch 事前学習 + ゲート付き融合を前提としており、この設定には移転しない。
5. 学習された `mid_loop_norm.weight` は 0.76〜1.22 (初期値 1) でほぼ動いておらず、注入信号の RMS は注入先残差の約 0.6 倍
   (既存実装は入力埋め込みの約 8 倍)。モデルは再帰信号を増幅する方向に学習していない。

### 次の候補 (未実施、結果を見て判断)

- 注入前に学習可能な射影/ゲート (T2MLR の Φ(h, r)) を挟み、出口層と入口層の空間対応を明示的に学習させる。
- 出口層の残差を ln_f + tied embedding 経由で「ソフトトークン」に射影してから入力空間に戻す (Coconut 型の対応を保つ)。
- 新規パラメータの学習率を上げる / ゲイン初期値を注入先のスケールに合わせる。
- Llama-3.2-1B での確認は、上の改善を入れた構成が GPT-2 で成立してからにする。
